...
export function initEzoic(): void {
  ...
  // This prevents the "Monetization not allowed for site" 403 error in development/preview
  const allowedDomains = ["bangerboard.com", "www.bangerboard.com"]
  const isAllowedDomain = allowedDomains.includes(window.location.hostname)
  const isExplicitlyEnabled = process.env.NEXT_PUBLIC_EZOIC_ENABLED === "true"

  if (!isAllowedDomain && !isExplicitlyEnabled) {
    console.log("[v0] Ezoic ads disabled in this environment to prevent 403 errors")
    return
  }
